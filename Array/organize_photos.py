...
# “<<photoname>>.<<extension>>, <<city_name>>, yyyy—mm—dd hh:n-m: ss , where "<<photoname>>", "<<extension>>" and "<<city_name>>"
# consist only of letters of the English alphabet and supply the name of the photo, the file name extension and the city name, respectively.
# Write a function:
# function solution (S) { // your code } that, given a string representing the list of M photos, returns the string representing the list of the new names of all photos (the order of photos should stay the same).
...
class Photo:
    def __init__(self, file_name, extension, city, date_taken, position):
        self.file_name, self.extension, self.city, self.date_taken, self.position = \
             file_name,      extension,      city,      date_taken,      position
        self.post_fix = None


def solution(S):
    # write your code in Python 3.6
    photos_strings = S.split("\n")
    photos = []
    cities = set()

    for position, photo_string in enumerate(photos_strings):
        photo_properties = re.match(r'(\w+)(\.\w+), (\w+), ([\d\- :]+)', photo_string).groups()
        photos.append(Photo(photo_properties[0], photo_properties[1], photo_properties[2], photo_properties[3], position))
        cities.add(photo_properties[2])

    for city in cities:
        photos_in_city = list(filter(lambda p: p.city == city, photos))
        no_of_photos   = len(photos_in_city)

        photos_in_city.sort(key=lambda x: x.date_taken)

        for index, photo in enumerate(photos_in_city):
            photo_index = str(index + 1)
            post_fix    = '0' * (len(str(no_of_photos)) - len(photo_index))

            photo.post_fix = post_fix + photo_index

    organized_photos_name = list(map(lambda p: p.city + p.post_fix + p.extension, photos))

    return '\n'.join(organized_photos_name)
